...
var $author$project$Tutorial$Init = {$: 'Init'};
...
var $author$project$Tutorial$init = function (_v0) {
	return A2(
		$author$project$Tutorial$update,
		$author$project$Tutorial$Init,
		{cards: $author$project$TutData$tutcards});
};
...
var $author$project$Tutorial$viewCard = function (c) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(c.explanation)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
...
var $author$project$Tutorial$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('cards-container'),
		...
		_List_fromArray(
			[
				...
				A2($elm$core$List$map, $author$project$Tutorial$viewCard, model.cards))
			]));
};
var $author$project$Tutorial$main = $elm$browser$Browser$element(
	{
		init: $author$project$Tutorial$init,
		subscriptions: function (_v0) {
			return $elm$core$Platform$Sub$none;
		},
		update: $author$project$Tutorial$update,
		view: $author$project$Tutorial$view
	});
...
var $author$project$BrightnessPort$Blue = {$: 'Blue'};
var $author$project$BrightnessPort$Dark = {$: 'Dark'};
var $author$project$BrightnessPort$Light = {$: 'Light'};
var $author$project$Editor$readBrMode = function (s) {
	switch (s) {
		case 'brightness-blue':
			return $author$project$BrightnessPort$Blue;
		case 'brightness-dark':
			return $author$project$BrightnessPort$Dark;
		case 'brightness-light':
			return $author$project$BrightnessPort$Light;
		default:
			return $author$project$BrightnessPort$Light;
	}
};
var $author$project$Editor$ResetInterpreterMsg = {$: 'ResetInterpreterMsg'};
var $author$project$Editor$brightnessModeAsClass = function (bmode) {
	switch (bmode.$) {
		case 'Dark':
					...
					return A2(
						...
						function (_v8) {
							...
							var envs = A2(
								...
								patts);
							var valS = $author$project$Interpreter$valToStr(vVal);
							var message = 'incomplete pattern (\"' + (A2($elm$core$String$left, 5, valS) + ((($elm$core$String$length(valS) > 5) ? '...\"' : '\"') + ' missing)'));
							return A2(
								$toastal$either$Either$andThen,
								function (finalCase) {
									return A2(
...
var $author$project$BrightnessPort$onBrightnessChange = _Platform_outgoingPort('onBrightnessChange', $elm$json$Json$Encode$string);
var $author$project$SaveCode$save = _Platform_outgoingPort('save', $elm$json$Json$Encode$string);
var $author$project$Editor$toggleBrightness = function (bmode) {
	switch (bmode.$) {
		case 'Dark':
			return $author$project$BrightnessPort$Light;
		case 'Light':
...
var $author$project$Editor$update = F2(
	function (msg, model) {
		...
		while (true) {
			switch (msg.$) {
				case 'Run':
					if (_Utils_eq(model.interpreterMsg, $author$project$InterpreterMsg$Ignore)) {
						...
						var $temp$msg = $author$project$Editor$ResetInterpreterMsg,
							$temp$model = model;
						msg = $temp$msg;
						model = $temp$model;
						continue update;
				...
				case 'ResetInterpreterMsg':
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{interpreterMsg: $author$project$InterpreterMsg$empty}),
						$elm$core$Platform$Cmd$none);
				case 'ToggleBrightnessMode':
					var newModel = _Utils_update(
						model,
						{
							brightnessMode: $author$project$Editor$toggleBrightness(model.brightnessMode),
							brightnessModeBtnEnabled: false
						});
					return _Utils_Tuple2(
						newModel,
						$author$project$BrightnessPort$onBrightnessChange(
							$author$project$Editor$brightnessModeAsClass(newModel.brightnessMode)));
				default:
					return _Utils_Tuple2(
						_Utils_update(
							model,
...
var $author$project$Editor$init = function (flgs) {
	return A2(
		$author$project$Editor$update,
		$author$project$Editor$UpdateCode(flgs.code),
		{
			brightnessMode: $author$project$Editor$readBrMode(flgs.brightnessModeS),
			brightnessModeBtnEnabled: true,
			code: '',
...
var $author$project$Editor$ToggleBrightnessMode = {$: 'ToggleBrightnessMode'};
var $author$project$Editor$materialIcon = F2(
	function (clss, s) {
		return A2(
			$elm$html$Html$div,
			_Utils_ap(
...
var $author$project$Editor$brightnessIconOf = function (bmode) {
	switch (bmode.$) {
		case 'Dark':
			return A2($author$project$Editor$materialIcon, _List_Nil, 'brightness_2');
		case 'Light':
			return A2($author$project$Editor$materialIcon, _List_Nil, 'wb_sunny');
		default:
			return A2($author$project$Editor$materialIcon, _List_Nil, 'brightness_4');
	}
};
...
var $author$project$Editor$viewBrightnessBtn = function (model) {
	return model.config.isRunnable ? A2(
		$elm$html$Html$button,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('brightness-btn'),
				$elm$html$Html$Events$onClick($author$project$Editor$ToggleBrightnessMode),
				$elm$html$Html$Attributes$class('noselect'),
				$elm$html$Html$Attributes$disabled(!model.brightnessModeBtnEnabled)
			]),
		_List_fromArray(
			[
				$author$project$Editor$brightnessIconOf(model.brightnessMode)
			])) : A2($elm$html$Html$div, _List_Nil, _List_Nil);
};
var $author$project$Editor$Run = {$: 'Run'};
var $author$project$Editor$viewRunBtn = function (model) {
	return model.config.isRunnable ? A2(
		$elm$html$Html$button,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				function () {
				var _v0 = model.interpreterMsg;
				if (_v0.$ === 'Ignore') {
					return A2($author$project$Editor$materialIcon, _List_Nil, 'play_arrow_outline');
				} else {
					return A2($author$project$Editor$materialIcon, _List_Nil, 'stop_outline');
				}
			}()
			])) : A2($elm$html$Html$div, _List_Nil, _List_Nil);
};
			...
			[
				...
				_List_fromArray(
					[
						...
						'brightness-mode display-none ' + $author$project$Editor$brightnessModeAsClass(model.brightnessMode))
					]),
				_List_Nil),
				$author$project$Editor$viewRunBtn(model),
				$author$project$Editor$viewBrightnessBtn(model),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$placeholder('write your code here...')
							]),
						_List_Nil),
						A2(
...
_Platform_export({'Editor':{'init':$author$project$Editor$main(
	A2(
				...
				function (conf) {
					return A2(
						...
						function (code) {
							return A2(
								$elm$json$Json$Decode$andThen,
								function (brightnessModeS) {
									return $elm$json$Json$Decode$succeed(
										{brightnessModeS: brightnessModeS, code: code, conf: conf, stdLibCode: stdLibCode});
								},
								A2($elm$json$Json$Decode$field, 'brightnessModeS', $elm$json$Json$Decode$string));
						},
		...
		A2($elm$json$Json$Decode$field, 'stdLibCode', $elm$json$Json$Decode$string)))(0)},'Tutorial':{'init':$author$project$Tutorial$main(
